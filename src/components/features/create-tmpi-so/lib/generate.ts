...
/** Wrapper murni atas window.qpilotApiFetch (client extension) atau
 * authorizedFetch (web app — PAT via session/proxy, sama seperti shared.tsx). */
async function confluenceRequest<T>(url: string, options: RequestInit = {}): Promise<T> {
  const doFetch = window.qpilotApiFetch ?? authorizedFetch;
  if (!doFetch) throw new Error("Authenticated API client belum dimuat.");
