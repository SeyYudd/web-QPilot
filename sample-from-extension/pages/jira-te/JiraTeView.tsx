...
export default function JiraTeView({
  ...
  const updateBulkStatus = async () => {
    ...
    setBulkResult("");
    try {
      const stored = await chrome.storage.local.get(["jiraUrl"]);
      const base = String(stored.jiraUrl || "").replace(/\/$/, "");
        ...
        result.keys.map(async (testKey) => {
          const issue = await client.request<any>(
           ...
           return {
             ...
             assignee: assignee?.displayName || assignee?.name || assignee?.accountId || (typeof runAssignee === "string" ? runAssignee : runAssignee?.displayName || runAssignee?.name) || "Unassigned",
             status: typeof run === "string" ? "TODO" : String(run?.status && typeof run.status === "object" ? run.status.name : run?.status || "TODO").toUpperCase(),
          };
        }),
      );
      // Bersihkan urutan basi dari build lama; tampilan selalu mengikuti urutan Xray/Jira.
      chrome.storage.local.remove(`qpilot_jira_te_order_${key.trim().toUpperCase()}`);
      setItems(next);
  ...
  const add = async () => {
    ...
    try {
      ...
      const issue = await createJiraClient(base).request<any>(`${base}/rest/api/2/issue/${encodeURIComponent(testKey)}?fields=issuetype`);
  ...
  const openDetail = async (item: (typeof items)[number]) => {
    const stored = await chrome.storage.local.get(["jiraUrl"]);
    const base = String(stored.jiraUrl || "https://jira.bri.co.id").replace(/\/$/, "");
    await (chrome as unknown as { tabs: { create(options: { url: string }): Promise<unknown> } }).tabs.create({ url: `${base}/browse/${encodeURIComponent(item.key)}` });
  };
  return (
    <>
    <Card>
      <CardHeader>
        ...
        <h2 className="mt-1 text-base font-serif font-semibold text-ink">
