// Logic bisnis: load Test Case dari Jira Test Execution (Xray).
import { getJiraBaseUrl, requestApi } from "../../../lib/api-request";
...
function extractIssueSteps(issue: any) {
  ...
  for (const id of [
    ...
    const rows = Array.isArray(value) ? value : value?.steps;
    if (!Array.isArray(rows)) continue;
    const mapped = rows
      .map((item: any) => {
        const source = item?.fields || item || {};
        return {
          action: stepText(
      ...
      .filter((step: any) => step.action || step.data || step.expectedResult);
...
export async function loadTestCases(
  ...
  const values = Array.isArray(list) ? list : list?.tests || list?.issues || [];
  const keys = [
    ...new Set(
      values
        .map((item: any) =>
          typeof item === "string" ? item : item?.key || item?.issueKey,
        )
  ...
  const testCases = await Promise.all(
    keys.map(async (key: string, index) => {
      ...
      const cleanSteps = steps.map((step: any) => ({
